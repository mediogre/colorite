...
void ConsoleTools::genOutput(bool useTokens, Writer *writer){
  try{
    // Source file text lines store.
    TextLinesStore textLinesStore;
    textLinesStore.loadFile(inputFileName, inputEncoding, true);
    ...
    if (htmlEscaping && !writer) delete commonWriter;
...
void ConsoleTools::genTokenOutput(Writer *writer){
  genOutput(true, writer);
}
